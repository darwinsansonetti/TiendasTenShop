...
@section('title', 'TiendasTenShop | Ventas por vendedores')
      ...
      <div class="col-sm-6">
        <div class="d-flex align-items-center gap-2">
          <div class="d-flex align-items-center justify-content-center rounded-2 me-1"
            ...
            <i class="bi bi-bar-chart text-white" style="font-size:1.1rem;"></i>
          </div>
          <div>
            <h4 class="mb-0 fw-bold text-dark" style="font-size:1.1rem;">Ventas por Vendedores</h4>
            <p class="mb-0 text-muted" style="font-size:0.78rem;">Historial de ventas por empleado</p>
          </div>
        </div>
      </div>
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-end">
          ...
          <li class="breadcrumb-item active" aria-current="page">Ventas por vendedores</li>
        </ol>
      </div>
    </div>
    <!--end::Row-->
        ...
        <div class="card">
            <div class="card-body p-0">
                <div class="table-responsive" style="max-height: 600px; overflow-y: auto;">
                    <table class="table table-hover mb-0" id="tablaVentasEmpleados">
                        <thead class="table-light">
                            <tr>
                                <th colspan="2" class="text-center">Empleado</th>
                                <th width="120" class="text-center">Fecha</th>
                                <th width="200" class="text-center">Sucursal</th>
                                <th width="100" class="text-center">Vendidos</th>
                                <th width="120" class="text-center">Venta USD</th>
                                <th width="120" class="text-center">Venta Bs</th>
                                <th width="120" class="text-center">Detalle</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($rankingVendedor as $index => $venta)                                
                                @php
                                    $fotoPerfil = $venta->Vendedor['FotoPerfil'] ?? '';
                                ...
                                <tr class="align-middle">
                                    ...
                                    <td class="fw-bold">
                                        @if($venta->Vendedor)
                                            ...
                                            <br>
                                            <small class="text-muted">{{ $venta->Vendedor['VendedorId'] }}</small>
                                        @else
                                            <em class="text-muted">Sin vendedor</em>
                                        @endif
                                    </td>
                                    ...
                                    <td class="text-center fw-bold">
                                        {{ $venta->SucursalNombre ?? 'N/A' }}
                                    </td>

                                    <!-- Vendidos (Cantidad) -->
                                    <td class="text-center fw-bold">
                                        {{ number_format($venta->Cantidad, 0, ',', '.') }}
                                    </td>

                                    <!-- Venta USD -->
                                    <td class="text-center fw-bold text-success">
                                        $ {{ number_format($venta->TotalDivisa, 2, ',', '.') }}
                                    </td>

                                    <!-- Venta Bs -->
                                    <td class="text-center fw-bold text-success">
                                        Bs. {{ number_format($venta->TotalBs, 2, ',', '.') }}
                                    </td>
