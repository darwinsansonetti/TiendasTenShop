...
@php
    ...
    $hdrIcon = 'pencil';
    $hdrTitle = 'Editar Punto de Venta';
    $hdrSubtitle = 'Modificar el punto de venta';
@endphp
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-header border-0 py-3" style="background:{{ $hdrBg }};">
                <h6 class="mb-0 fw-bold text-white">
                    <i class="bi bi-pencil me-2"></i>Editar Punto de Venta
                </h6>
            </div>
            <div class="card-body">
                <form action="{{ route('cpanel.puntos.actualizar', $punto->PuntoDeVentaId) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="banco_id" id="banco_id" class="form-select @error('banco_id') is-invalid @enderror" required>
                                ...
                                @foreach($bancos as $banco)
                                    <option value="{{ $banco->ID }}" {{ old('banco_id', $punto->BancoId) == $banco->ID ? 'selected' : '' }}>
                                        {{ $banco->Nombre }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="sucursal_id" id="sucursal_id" class="form-select @error('sucursal_id') is-invalid @enderror" required>
                                ...
                                @foreach($sucursales as $sucursal)
                                    <option value="{{ $sucursal->ID }}" {{ old('sucursal_id', $punto->SucursalId) == $sucursal->ID ? 'selected' : '' }}>
                                        {{ $sucursal->Nombre }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="descripcion" id="descripcion"
                                   ...
                                   value="{{ old('descripcion', $punto->Descripcion) }}" required>
                            @error('descripcion')
                                <div class="text-danger small">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="es_activo" id="es_activo" class="form-select @error('es_activo') is-invalid @enderror">
                                <option value="1" {{ old('es_activo', $punto->EsActivo) == 1 ? 'selected' : '' }}>Activo</option>
                                <option value="0" {{ old('es_activo', $punto->EsActivo) == 0 ? 'selected' : '' }}>Inactivo</option>
                            </select>
                            @error('es_activo')
                                <div class="text-danger small">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="mt-4 pt-2 d-flex gap-2" style="border-top:1px solid #f1f5f9;">
                        <button type="submit" class="btn px-4 fw-semibold text-white" style="background:{{ $hdrBg }};border:none;">
                            <i class="bi bi-save me-1"></i> Actualizar Punto de Venta
                        </button>
